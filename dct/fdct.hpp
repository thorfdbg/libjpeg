...
**
** $Id: fdct.hpp,v 1.7 2012-06-02 10:27:14 thor Exp $
**
*/
...
/// class FDCT
// This class implements the floating point DCT.
class FDCT : public DCT {
  //
  // The (inverse) quantization tables, i.e. multipliers.
  FLOAT m_pfInvQuant[64];
  //
  // The quantizer tables.
  FLOAT m_pfQuant[64];
  //
  // Quantize a floating point number with a multiplier, round correctly.
  static inline LONG Quantize(FLOAT n,FLOAT qnt)
  {
    // There are surely faster ways of doing this. But never mind...
    return (n >= 0.0)?LONG(n * qnt + 0.5):LONG(n * qnt - 0.5);
  }
  //
public:
  FDCT(class Environ *env);
  //
  ~FDCT(void);
  //
