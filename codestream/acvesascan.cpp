...
** This is the arithmetic coded version.
**
...
#include "codestream/acvesascan.hpp"
#include "marker/scan.hpp"
#include "control/linebuffer.hpp"
#include "control/bufferctrl.hpp"
///

/// ACVesaScan::ACVesaScan
// Create a new scan. This is only the base type.
ACVesaScan::ACVesaScan(class Frame *frame,class Scan *scan,
...
/// ACVesaScan::~ACVesaScan
// Dispose a scan.
ACVesaScan::~ACVesaScan(void)
{
}
///


/// ACVesaScan::FindComponentDimensions
...
void ACVesaScan::FindComponentDimensions(void)
{ 
  UBYTE cx;
  ...
  if (m_ucCount > 4)
    JPG_THROW(OVERFLOW_PARAMETER,"ACVesaScan::FindComponentDimensions",
  ...
  for(cx = 0;cx < m_ucCount;cx++) {
    ...
    if (comp->MCUHeightOf() != 1 || comp->MCUWidthOf() != 1)
      JPG_THROW(INVALID_PARAMETER,"ACVesaScan::FindComponentDimensions",
...
/// ACVesaScan::ConvertToComplement
...
void ACVesaScan::ConvertToComplement(struct Line *line[4],bool abort,UBYTE abortcx,UBYTE abortlevel,UBYTE abortbitlevel)
{
  UBYTE cx;
...
bool ACVesaScan::ParseMCU(void)
{
  ...
  do {
    ...
    //
    BeginReadMCU(m_Coder.ByteStreamOf()); 
    m_ulUsedBits = 0;
    //    
    // Decode the levels
    //debug = (linecounter >= 19) || (linecounter <= 24);
    {
      ...
      for(cx = 0;cx < m_ucCount;cx++) {
	bitlevel = 0;
	while((m_ulUsedBits++,m_Coder.Get(m_Contexts[cx].C)) == 0)
	  bitlevel++;
	...
	do {
	  ...
	  for(cx = 0;cx < m_ucCount;cx++) {
		...
		DecodeEZWLevel(m_Contexts[cx],(ULONG *)(line[cx]->m_pData),m_ulWidth[cx],increment,1 << bitlevel,lowpass);
	      }
	    }
	  }
...
bool ACVesaScan::WriteMCU(void)
{
  ...
  do {
	...
	for(int i = 0;i < bitplane[cx];i++)
	  m_Coder.Put(m_Contexts[cx].C,false);
	if (bitplane[cx] > maxbits)
	  maxbits = bitplane[cx];
	m_Coder.Put(m_Contexts[cx].C,true);
	...
	do {
	  ...
	  for(cx = 0;cx < m_ucCount;cx++) {
		...
		//if (debug) printf("Line : %d,%d,%d,%d\n",linecounter,cx,increment,bitlevel);
		EncodeEZWLevel(m_Contexts[cx],(ULONG*)(line[cx]->m_pData),m_ulWidth[cx],increment,1 << bitlevel,lowpass);
	      }
	    }
	  }
	} while(abort == false && level--);
...
/// ACVesaScan::StartParseScan 
// Fill in the tables for decoding and decoding parameters in general.
void ACVesaScan::StartParseScan(class ByteStream *io,class BufferCtrl *ctrl)
{
...
/// ACVesaScan::StartWriteScan
// Begin writing the scan data
void ACVesaScan::StartWriteScan(class ByteStream *io,class BufferCtrl *ctrl)
{
...
void ACVesaScan::Restart(void)
{
  m_Coder.OpenForRead(m_Stream.ByteStreamOf()); 
  for(int i = 0;i < 4;i++) {
    m_Contexts[i].Init();
  }
}
